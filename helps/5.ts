...
const jsonBodyMiddleware = express.json(); //es nra hamara vos json karena karda toli esiminch
...
app.get("/courses", (req, res) => {
  ...
  if (req.query.title) {
    ...
    foundCourses = foundCourses.filter(
      //filter ani kursery
      (c) => c.title.indexOf(req.query.title as string) > -1 //prosty tenum enq query title i bary mer kursi titlum ka te che
    );
  }
